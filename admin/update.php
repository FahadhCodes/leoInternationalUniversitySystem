  ...
  <div class="container-fluid">
    ...
    <form action="" method="post" class="my-3">
      <div class="formContainer">
        ...
        if (isset($_POST['cb-1'])) {
          ...
          echo "<script>alert('FACULTY NAME UPDATED (Click the Side bar Update Button to see the tabel preview)')</script>";
        }
        ?>
        <!--------------------------------------------------------------------------DEPARTMENT----------------------------------------------------------------------->
        ...
        if (isset($_POST['cb-2'])) {
          ...
          mysqli_query($con, $updateQuery);
          echo "<script>alert('DEPARMENT NAME UPDATED')</script>";
        }
        ?>
        <!--------------------------------------------------------------------------SUBJECTS----------------------------------------------------------------------->
        <h3>Edit Subject</h3>
      ...
      if (isset($_POST['cb-3'])) {
        ...
        if ($year) {
          ...
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Subject Year Updated')</script>";
        }
        if ($sem) {
          $updateQuery = "UPDATE `subject` SET `semester`='$sem' WHERE `subject_id`='$sid'";
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Subject Semester Updated')</script>";
        }
        if ($sub) {
          $updateQuery = "UPDATE `subject` SET `subject_name`='$sub' WHERE `subject_id`='$sid'";
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Subject Name Updated')</script>";
        }
      }
      ?>
    </form>
    ...
    <form action="" method="post" class="my-3 STUDENTS" enctype="multipart/form-data">
      ...
      if (isset($_POST['submit-A'])) {
        ...
        if ($faculty = $_POST['facultySelection3']) {
          ...
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Student Faculty Updated')</script>";
        }
        if ($aYear = $_POST['aYear']) {
          ...
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Student Academic Year Updated')</script>";
        }
        if ($Department = $_POST['DepartmentSelector1']) {
          ...
          mysqli_query($con, $updateQuery);
          echo "<script>alert('Students Department Updated')</script>";
        }
        if ($fname = $_POST['fname'] || $lname = $_POST['lname'] || $_FILES['prof-std']) {
          ...
          if ($img_name = $_FILES['prof-std']['name']) {
            ...
            if (in_array(strtolower($ext), ["png", "jpg", "jpeg"])) {
              ...
              echo "<script>alert('STUDENT PROFILE PICTURE UPDATED')</script>";
            } else if ($ext == "") {
              return;
            } else {
    ...
    if (isset($_POST['stfID'])) {
      ...
      if (!empty($_POST['fname_L']) && !empty($_POST['lname_L'])) {
        ...
        echo "<script>alert('STAFFS FIRST AND LAST NAME UPDATED')</script>";
        ...
        echo "<script>alert('STAFFS FIRST NAME UPDATED')</script>";
      ...
      if ($gender = $_POST['gender'] ?? "") {
        ...
        echo "<script>alert('STAFFS GENDER UPDATED')</script>";
      }
      if ($_POST['role'] != "#") {
        $role = $_POST['role'];
        mysqli_query($con, $updateQuery);
        ...
        if ($_POST['role_A'] != "#") {
          ...
          mysqli_query($con, $updateQuery);
          staffImageFileRenamer($ID, $row['staff_fname'], $row['staff_lname'], $ext, $oldImageFileName);
          echo "<script>alert('STAFFS ACADEMIC ROLE UPDATED')</script>";
          ...
          $updateQuery = "UPDATE `staffs` SET `staffID`='$ID', `role`='$role', `sub_role`='$role_NA',`profile_pic`='$IMG_FILE_NAME' WHERE `staffID`='$stfID'";
          mysqli_query($con, $updateQuery);
          echo "<script>alert('STAFFS NON-ACADEMIC ROLE UPDATED')</script>";
        }
      }
      //IMAGE----------------------------------->
      if ($_FILES['prof_lc']['name'] != "") {
        ...
        if (in_array(strtolower($ext), ["png", "jpg", "jpeg"])) {
          move_uploaded_file($img_temp_path, $profil_pic_path);
          echo "<script>alert('STAFFS PROFILE PICTURE UPDATED')</script>";
        } else if ($ext == "") {
          return;
        } else {
          echo "<script>alert('UPLOADED FILE IS NOT AN IMAGE!')</script>";
